	...
	if ($listings->have_posts()) {
		...
		while ($listings->have_posts()) {
			...
			$newListings[] = array(
				...
				'price' => (!empty($price)) ? str_replace('   ', ' ', stm_listing_price_view( esc_html__('No Price', 'stm-motors-application'), trim( $price ) ) ) : 'No Price',
				'grid' => array(
					'title' => $genTitle,
...
function removeDuplicates( $array, $key ) {
	...
	foreach ( $array as $item ) {
		// Extract the first part of the file name before the underscore
		$urlParts      = pathinfo( $item[ $key ] );
		$fileNameParts = explode( '_', $urlParts['filename'] );
		$value         = $fileNameParts[0];
		// Check if the value has already been encountered
		if ( ! in_array( $value, $uniqueKeys ) ) {
			// Add the value to the unique keys array and add the item to the result array
			$uniqueKeys[] = $value;
			$result[]     = $item;
		}
	}
...
function stm_mra_get_listing_by_id()
{
	...
	if (!empty($listing)) {
		...
		if ( ! empty( $featureImg ) ) {
			array_unshift( $gallery, array( 'url' => $featureImg ) );
		}
		$gallery = removeDuplicates($gallery, "url");
		$response = array(
			'ID' => $id,
			'imgUrl' => $featureImg,
			'gallery' => $gallery,
			'imgCount' => count($gallery),
			'price' => (!empty($price)) ? str_replace('   ', ' ', stm_listing_price_view( esc_html__('No Price', 'stm-motors-application'), trim( $price ) ) ) : esc_html__('No Price', 'stm-motors-application'),
			'title' => $genTitle,
			'subTitle' => $genSubTitle,
			...
			'car_lng' => $carLng,
			'info' => array(),
			'vin' => get_post_meta( $id, 'vin_number', true ),
		);
		foreach ($info as $inf) {
